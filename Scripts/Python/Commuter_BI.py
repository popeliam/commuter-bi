...
conn = psycopg2.connect(host='localhost',database='postgres',user='postgres',password='your-password')

# data = requests.get(toWork).json()
...
def req(o,d):
	global url,key
	reqUrl = url + 'origin=' + o + '&destination='+ d + '&departure_time=now&key=' + key
